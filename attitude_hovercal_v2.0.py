...
if checks:
    ...
    for test_img in ti:
        ...
        for xx,yy in zip(points_2D[test_img-start_frame,:,0],points_2D[test_img-start_frame,:,1]):
            cv.drawMarker(temp, (int(xx), int(yy)),(0,255,0), markerSize=20, markerType=cv.MARKER_SQUARE, thickness=2, line_type=cv.LINE_AA)
...
#compare gps and photogrammetry tvec after alliniament
if checks and savefig:
    plt.figure(figsize=(8,8))
...
if bootstrap:
    ...
    for k in tqdm(range(n_res)):
        ...
        res = []
        rvec_new = []
        for rv, t, p2D in zip(rvecs, gps_cam, points_2D):
    ...
    for y in range(length):
        ...
        #meanYawFrame.append(np.mean(bootYPR[:,y,0]))
        stdYawFrame.append(np.std(bootYPR[:,y,0]))

        #meanPitchFrame.append(np.mean(bootYPR[:,y,1]))
        stdPitchFrame.append(np.std(bootYPR[:,y,1]))

        #meanRollVFrame.append(np.mean(bootYPR[:,y,2]))
        stdRollVFrame.append(np.std(bootYPR[:,y,2]))

        #meanRollHFrame.append(np.mean(bootYPR[:,y,3]))
        stdRollHFrame.append(np.std(bootYPR[:,y,3]))

    bootError = np.array([stdYawFrame,stdPitchFrame,stdRollVFrame,stdRollHFrame])

    #print(bootError.shape)
    #print(bootError)
    #print(stdPitchFrame)

    if checks and savefig:
       plt.figure(figsize=(8,8))
       labels=['Yaw_gauss','Pitch_gauss', 'Roll_v_gauss', 'Roll_h_gauss']
       for w in range(4):
            plt.subplot(2,2,w+1)
            plt.title(labels[w])
            plt.hist(bootError[w],bins=10,density=True)
            plt.plot(np.linspace(np.min(bootError[w]), np.max(bootError[w]),len(bootError[w])),
                    norm.pdf(np.linspace(np.min(bootError[w]), np.max(bootError[w]),len(bootError[w])),np.mean(bootError[w]),np.std(bootError[w])))

            
       plt.savefig(outpath+'bootstrapGaussianity.png')       
...
if jackknife:
    ...
    for y in range(length):
        ...
        #meanYawFrame.append(np.mean(bootYPR[:,y,0]))
        stdYawFrame.append(np.std(jackYPR[:,y,0]))

        #meanPitchFrame.append(np.mean(bootYPR[:,y,1]))
        stdPitchFrame.append(np.std(jackYPR[:,y,1]))

        #meanRollVFrame.append(np.mean(bootYPR[:,y,2]))
        stdRollVFrame.append(np.std(jackYPR[:,y,2]))

        #meanRollHFrame.append(np.mean(bootYPR[:,y,3]))
        stdRollHFrame.append(np.std(jackYPR[:,y,3]))

    jackError = np.array([stdYawFrame,stdPitchFrame,stdRollVFrame,stdRollHFrame])
    ...
    if checks and savefig:
       ...
       for w in range(3):
            plt.subplot(2,2,w+1)
            plt.title(labels[w])
            plt.hist(jackError[w],bins=10,density=True)
            plt.plot(np.linspace(np.min(jackError[w]), np.max(jackError[w]),len(bootError[w])),
                    norm.pdf(np.linspace(np.min(jackError[w]), np.max(jackError[w]),len(jackError[w])),np.mean(jackError[w]),np.std(jackError[w])))

            
       plt.savefig(outpath+'jackknifeGaussianity.png')       

#print(jackError.shape)
#print(jackError)
########################################################################################################################################
